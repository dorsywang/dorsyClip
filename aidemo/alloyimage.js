        ...
        var M = {
            getFun: function(fun){
                var Effects = {
                    ...
                    e15: function(){
                        var m = this.clone().act("曲线", [0, 30, 50, 128, 180, 200, 225], [0, 100, 100, 240, 240, 240, 225]).act("灰度阈值");
                        return m.add(
                            m.clone().act("反色").act("setHSI", 52, 43, -33, 1),"差值"
                        );
                    }
                };
...
;(function(Ps){
    ...
    window[Ps].module("tools",function(P){
        ...
        var M = {
            getTools: function(type, args){
                switch(type){
                    
                    case "brush":
                        return this.brush(args);
                }
            ...
            //(type, 大小, 颜色, 硬度)
            brush: function(args){
                var brushType = args[0].toLocaleLowerCase(),
                    size = args[1],
                    color = args[2],
                    hardness = args[1];
